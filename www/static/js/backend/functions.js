...
function addNewRow() {
    // copy last node
    var newRow = document.querySelector('#users tbody tr:last-child').cloneNode(true);
    document.querySelector('#users tbody').appendChild(newRow);
    // change add/del links
    var allRows = document.querySelectorAll('.new_user');
    ...
    // delete all values
    var perentTr = allRows[allRows.length - 1].parentNode.parentNode;
